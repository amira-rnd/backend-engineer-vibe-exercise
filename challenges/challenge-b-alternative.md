...
**Excellent (5/5):**
- Identifies all 4+ memory leaks
- Fixes unbounded cache growth
- Cleans up event listeners
- Implements connection pooling
- Addresses recursive depth issue
- Adds proper error recovery

**Good (4/5):**
- Identifies most memory leaks
- Implements cache limits
- Fixes connection cleanup
- Shows good debugging methodology

**Acceptable (3/5):**
- Recognizes memory issues
- Fixes some problems
- Basic understanding shown

### Memory Leak Sources (For Interviewer Reference)

1. **Unbounded cache** - Never cleaned, grows forever
2. **Event listener accumulation** - Listeners added but never removed
3. **Connection pool growth** - activeConnections Map never cleaned
4. **Axios instances** - New instance per assessment, interceptors pile up
5. **Processing queue** - Failed assessments accumulate
6. **Circular references** - Cache stores connections that reference cache
7. **Recursive depth** - Can cause stack overflow

### Discussion Questions

1. How would you monitor this in production?
2. What patterns prevent these issues?
3. How would you implement cache eviction?
4. What's the trade-off between caching and memory?
5. How would you handle the rate limiting properly?

### What NOT to Use AI For

Candidates should:
- ❌ NOT ask AI "what's causing the memory leak?"
- ❌ NOT ask AI to find all issues
- ✅ Identify patterns themselves, then ask for implementation
- ✅ Understand resource lifecycle before fixing

### Alternative Technologies

If candidate prefers, they can implement in:
- Python (with asyncio and connection pools)
- Java (with thread pools and connection management)
- Go (with goroutines and channel management)

The key is testing debugging methodology, not language specifics.
